...
GifPerformance.prototype = extend({
  ...
  getTextFrameData: function (text, cb) {
    ...
    var fn = (drawImage + '')
              ...
              .replace('"FONT_SIZE"', JSON.stringify('30'))
              .replace('"FONT_FAMILY"', JSON.stringify('Impact'))
              .replace('"TEXT"', JSON.stringify(text))
              .replace('"BACKGROUND"', JSON.stringify('#FF00FF'))
              .replace('"FOREGROUND"', JSON.stringify('#FFFFFF'))
