...
ByteArray.prototype.startTransaction = function (namespace) {
  var id = this.transactionCounter;
  ...
  this.emit('startTransaction', transaction);
  return id;
};
ByteArray.prototype.stopTransaction = function (id) {
  var transaction = this.transactions[id];
  assert(transaction, 'Transaction "' + id + '" could not be found');

  // Clean up transaction
  var activeTrans = this.activeTransactions;
  activeTrans.splice(activeTrans.indexOf(transaction), 1);
  delete this.transactions[id];

  // Emit transaction and namespace events
  var buffer = new Buffer(transaction.data);
  this.emit('stopTransaction', transaction);
  this.emit(transaction.namespace, buffer);
};
...
ByteArray.prototype.writeByte = function(val) {
  ...
  this.emit('byte', buffer);
  this.emit('data', buffer);

  // Output to the active transaction buffers
  this.activeTransactions.forEach(function (transaction) {
    transaction.data.push(val);
  });
};
...
GIFEncoder.prototype.addFrame = function(imageData) {
  var transId = this.startTransaction('frame');
